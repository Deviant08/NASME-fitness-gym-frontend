...
async function api(file, method = 'GET', body = null, params = {}) {
  ...
  let res;
  try { res = await fetch(url, opts); } catch (err) { throw new Error('Failed to fetch — check network or backend status'); }
...
function navigate(page) {
  ...
  const titles = { dashboard: 'DASHBOARD', members: 'MEMBER MANAGEMENT', staff: 'STAFF MANAGEMENT', payments: 'PAYMENTS & SUBSCRIPTIONS', equipment: 'EQUIPMENT MANAGEMENT', security: 'DATA & SECURITY' };
  ...
  try {
    ...
    if (!activity.length) {
      try {
        ...
        activity = (audit.data || []).map(a => ({ action: a.action, actor: a.by_user || a.actor || 'System', logged_at: a.logged_at || a.created_at, color_tag: a.color_tag }));
      } catch (_) {}
    }
...
async function loadMembers(q = '') {
  try {
    const params = { q };
    if (memberStatusFilter) params.status = memberStatusFilter;
    const json = await api('members.php', 'GET', null, params);
...
async function loadStaff() {
  ...
  try {
    ...
    tbody.innerHTML = data.length === 0 ? '<tr><td colspan="5" class="empty">No staff accounts yet.</td></tr>' : data.map(s => `
      <tr><td>${s.username}</td><td>${s.full_name}</td><td>${s.role}</td>
      <td>${formatDate(s.created_at)}</td>
      <td>${s.role === 'superadmin' ? '' : `<button class="btn-sm btn-ghost" data-remove-staff="${s.id}">Remove</button>`}</td></tr>`).join('');
  ...
  tbody.innerHTML = data.map(m => `
    <tr data-id="${m.id}">
      ...
      <td class="actions">
        ...
        ${(m.status || '') === 'Archived'
          ? `<button class="btn-sm btn-ghost" data-action="restore" data-id="${m.id}">Restore</button>`
...
function renderActivityFeed(data = []) {
  ...
  if (!feed) return;
  if (!data.length) { feed.innerHTML = '<div class="empty-state">No recent activity yet.</div>'; return; }
  feed.innerHTML = data.slice(0, 15).map(a => {
    ...
    return `<div class="activity-item"><div class="activity-action">${action}</div>
...
function previewHtml(m) {
  return `<h3>${dash(m.full_name)} <small>(${dash(m.member_code)})</small></h3>
...
async function handleMemberAction(action, id) {
  ...
  if (action === 'restore') {
    ...
    try {
      await api('members.php?id=' + encodeURIComponent(id), 'PUT', { restore: true, status: 'Active' });
...
async function saveMember(formData) {
  try {
    ...
    delete formData.id;
    if (formData.plan) {
      formData.expires_at = computeExpiresClient(formData.plan, formData.start_date || new Date().toISOString().slice(0, 10));
    }
...
function computeExpiresClient(plan, start) {
  ...
  if (p.includes('day')) return start;
...
function initApp() {
  ...
  document.querySelectorAll('#member-status-filters .filter-pill').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('#member-status-filters .filter-pill').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      memberStatusFilter = btn.dataset.status || '';
      loadMembers(document.getElementById('member-search')?.value.trim() || '');
    });
  });
  checkSession();
